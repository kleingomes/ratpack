...
public class DefaultHttpClient implements HttpClientInternal {
  ...
  public Promise<ReceivedResponse> request(URI uri, final Action<? super RequestSpec> requestConfigurer) {
    return intercept(
      Promise.async(downstream -> new ContentAggregatingRequestAction(uri, this, 0, false, Execution.current(), requestConfigurer.append(spec.requestInterceptor)).connect(downstream)),
  ...
  public Promise<StreamedResponse> requestStream(URI uri, Action<? super RequestSpec> requestConfigurer) {
    return intercept(
      Promise.async(downstream -> new ContentStreamingRequestAction(uri, this, 0, false, Execution.current(), requestConfigurer.append(spec.requestInterceptor)).connect(downstream)),
      spec.responseInterceptor,
      spec.errorInterceptor
    );
  }
